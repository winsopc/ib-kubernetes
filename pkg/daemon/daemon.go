...
//nolint:nilerr,funlen // legacy add path; complexity acknowledged, partition-aware fast path runs before this
func (d *daemon) AddPeriodicUpdate() {
	...
	for networkID, podsInterface := range addMap.Items {
		...
		// Partition-managed NADs use hardware PF GUIDs from the FabricClient backend;
		// non-partition NADs fall through to the legacy GUID/PKey flow below.
...
func (d *daemon) setupPartitionForNAD(nad *v1.NetworkAttachmentDefinition) error {
	...
	if parseErr != nil {
		...
		return nil //nolint:nilerr // intentional graceful skip: parseErr means "not relevant", not a failure
	}
