...
type PartitionReader interface {
	...
	DeletePartitionPods(networkID string, pods []*kapi.Pod)
}
...
// hasNADFinalizer reports whether the NAD carries the given finalizer.
...
func (p *partitionController) processNADResults(addedNADs, deletedNADs *utils.SynchronizedMap) {
	...
	if p.fabricClient != nil && deletedNADs != nil {
		...
		for _, entry := range pendingDeletes {
			if !hasNADFinalizer(entry.nad, utils.PartitionNADFinalizer) {
...
func (p *partitionController) DeletePartitionPods(networkID string, pods []*kapi.Pod) {
	if p.fabricClient == nil {
