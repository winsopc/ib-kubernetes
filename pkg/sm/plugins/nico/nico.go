...
func newNicoPlugin() (*nicoPlugin, error) {
	...
	// When using NGC production API, Site ID and VPC ID must be explicitly provided.
	// Internal cluster URLs support auto-detection.
	if strings.Contains(strings.ToLower(conf.BaseURL), "api.ngc.nvidia.com") {
		if conf.SiteID == "" || conf.VpcID == "" {
			log.Error().Msgf("NICO_BASE_URL is NGC production API: NICO_SITE_ID and NICO_VPC_ID are required")
			if conf.SiteID == "" {
				log.Error().Msg("Missing NICO_SITE_ID")
			}
			if conf.VpcID == "" {
				log.Error().Msg("Missing NICO_VPC_ID")
			}
			return nil, fmt.Errorf(
				"nico plugin configuration is incomplete: NICO_SITE_ID and NICO_VPC_ID are required when using NGC production API")
		}
	}
	...
	// C2: Only override Site/VPC IDs when the operator explicitly provides them.
	// Authenticate() calls apiMetadata.Initialize() which auto-detects Site and VPC;
	// we apply overrides before Authenticate so they take precedence if set.
	if conf.SiteID != "" {
	...
	// C3: Wrap init sequence in a deadline context.
	// Auth + partition lookup = 2-3 API calls; 30 seconds is ample.
...
func (c *nicoPlugin) AddGuidsToPKey(pkey int, guids []net.HardwareAddr) error {
	...
	// Look up partitionID from pkey cache
	partitionIDVal, ok := c.pkeyToPartitionCache.Load(pkeyStr)
	if !ok {
		return fmt.Errorf("no partition cached for pkey %s; partition may not be ready yet", pkeyStr)
	}
...
func (c *nicoPlugin) processNodeAsync(nodeName string, op *pendingOp) {
	...
	if err := c.ibAssociateOnNode(nodeName, partitionID); err != nil {
		op.mu.Lock()
		op.err = fmt.Errorf("ibAssociateOnNode failed for node '%s': %w", nodeName, err)
		op.mu.Unlock()
		log.Warn().Str("node", nodeName).Str("partition_id", partitionID).Err(err).
	...
	// Step 2: Poll until IB interfaces are Ready or timeout
